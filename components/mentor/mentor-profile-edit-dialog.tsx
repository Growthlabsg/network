...
import { getMentorProfileCompletion } from '@/lib/mentor-profile-completion';
...
export interface MentorProfileEditDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mentor: Mentor | null;
  onSave: (data: Partial<Mentor>) => void;
}
...
export function MentorProfileEditDialog({ open, onOpenChange, mentor, onSave }: MentorProfileEditDialogProps) {
  ...
  const [status, setStatus] = useState<'Available' | 'Busy' | 'Limited'>('Available');
  ...
  useEffect(() => {
    ...
    setName(mentor.name);
    setTagline(mentor.tagline ?? '');
    setRole(mentor.role ?? '');
    setLocation(mentor.location);
    setDescription(mentor.description);
    setExpertiseText(listToText(mentor.expertise));
    setIndustriesText(listToText(mentor.industries));
    setAvailability(mentor.availability ?? '');
    setHourlyRate(mentor.hourlyRate ?? '');
    setExperience(mentor.experience ?? '');
    setStatus(mentor.status ?? 'Available');
    setAbout(mentor.about ?? '');
    setHowToBook(mentor.howToBook ?? '');
    setRecognitionText(listToText(mentor.recognition));
    setFaqs(faqsToEditable(mentor.faqs));
    setWebsite(mentor.website ?? '');
    setLinkedIn(mentor.linkedIn ?? '');
    setLogo(mentor.logo ?? '');
    setBanner(mentor.banner ?? '');
  }, [mentor, open]);
  ...
  const completion = useMemo(() => {
    ...
    const data: Partial<Mentor> & { logo?: string; banner?: string } = {
      name: name.trim() || undefined,
      tagline: tagline.trim() || undefined,
      role: role.trim() || undefined,
      location: location.trim() || undefined,
    ...
    return getMentorProfileCompletion(data);
  }, [name, tagline, role, location, description, expertiseText, industriesText, availability, hourlyRate, experience, about, howToBook, recognitionText, faqs, website, linkedIn, logo, banner]);
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          <div className="mb-6 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30 p-4">
            ...
            {!completion.isComplete && (
              <p className="text-xs text-amber-700 dark:text-amber-400 mt-2">Fill all required fields so your profile is complete and visible to founders.</p>
            )}
          </div>
          ...
          {section('Basics', (
            <>
              ...
              <Input value={tagline} onChange={(e) => setTagline(e.target.value)} className="mb-3" placeholder="e.g. Former Google Engineering Director" />
              <label className="block text-xs font-medium text-muted mb-1">Current role / company</label>
              <Input value={role} onChange={(e) => setRole(e.target.value)} className="mb-3" placeholder="e.g. TechStart Ventures" />
                ...
                <div className="flex flex-wrap items-start gap-4">
                  <div className="flex flex-col items-center gap-2">
                    <div className="h-24 w-24 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-600 overflow-hidden bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center">
                      {logo ? <img src={logo} alt="Logo preview" className="h-full w-full object-cover" /> : <User className="h-10 w-10 text-slate-400" />}
                    </div>
          ...
          {section('Expertise', (
            <>
              <label className="block text-xs font-medium text-muted mb-1">Expertise areas (one per line or comma-separated)</label>
              <textarea value={expertiseText} onChange={(e) => setExpertiseText(e.target.value)} rows={3} className="w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm mb-3" placeholder="AI/ML\nProduct Strategy\nScaling" />
              <label className="block text-xs font-medium text-muted mb-1">Industries</label>
              <Input value={industriesText} onChange={(e) => setIndustriesText(e.target.value)} placeholder="Tech, FinTech, SaaS" className="mb-3" />
              <label className="block text-xs font-medium text-muted mb-1">Availability</label>
              <Input value={availability} onChange={(e) => setAvailability(e.target.value)} placeholder="e.g. 1:1 sessions, Group workshops" className="mb-3" />
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  <label className="block text-xs font-medium text-muted mb-1">Hourly rate</label>
                  <Input value={hourlyRate} onChange={(e) => setHourlyRate(e.target.value)} placeholder="$500/hour" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-muted mb-1">Experience</label>
                  <Input value={experience} onChange={(e) => setExperience(e.target.value)} placeholder="15+ years" />
                </div>
              </div>
              <label className="block text-xs font-medium text-muted mb-1">Status</label>
              <select value={status} onChange={(e) => setStatus(e.target.value as 'Available' | 'Busy' | 'Limited')} className="w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm">
          ...
          {section('About & how to book', (
            <>
              ...
              <label className="block text-xs font-medium text-muted mb-1">How to book</label>
