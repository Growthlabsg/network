...
import { getTeacherById, getSimilarTeachers, getPeopleAlsoViewedTeachers, type Teacher } from '@/lib/mock-teachers';
import { getMyTeacherFromStorage, setMyTeacherInStorage } from '@/lib/teacher-me';
...
export default function TeacherDetailPage() {
  ...
  const [myTeacher, setMyTeacher] = useState<Teacher | null | undefined>(undefined);
  const sourceTeacher = useMemo(() => {
    if (id === 'me') return myTeacher ?? null;
    return id ? getTeacherById(id) : undefined;
  }, [id, myTeacher]);
  const [editedData, setEditedData] = useState<Partial<Teacher> | null>(null);
  const teacher = useMemo<Teacher | undefined>(() => {
    if (!sourceTeacher) return undefined;
    if (!editedData) return sourceTeacher;
    return { ...sourceTeacher, ...editedData } as Teacher;
  }, [sourceTeacher, editedData]);
  const similar = teacher && teacher.id !== 'me' ? getSimilarTeachers(teacher) : [];
  const peopleAlsoViewed = id && id !== 'me' ? getPeopleAlsoViewedTeachers(id) : [];

  useEffect(() => {
    if (id === 'me') setMyTeacher(getMyTeacherFromStorage());
  }, [id]);

  useEffect(() => {
    if (id === 'me' && myTeacher === null) router.replace('/teachers/create');
  }, [id, myTeacher, router]);
      ...
      <div className="max-w-md mx-auto py-16 px-4 text-center">
        <Card className="rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/20">
          <CardContent className="p-10">
            ...
            <h2 className="text-lg font-semibold text-growthlab-slate mb-2">Teacher not found</h2>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-8 pb-24 md:pb-0">
      <nav className="flex items-center gap-2 text-sm text-muted" aria-label="Breadcrumb">
        ...
        <span className="font-medium text-growthlab-slate truncate">{teacher.name}</span>
      </nav>
      ...
      <section className="relative overflow-hidden rounded-t-2xl shadow-sm" aria-hidden>
        {teacher.banner ? (
          <div className="aspect-[3/1] min-h-[140px] max-h-[200px] w-full">
            <img src={teacher.banner} alt="" className="h-full w-full object-cover" />
          </div>
        ) : (
            ...
            <div>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-growthlab-slate">{teacher.name}</h1>
              ...
              <p className="text-muted mt-2 max-w-2xl">{teacher.description}</p>
              <div className="flex flex-wrap items-center gap-2 mt-3">
                ...
                <span className="flex items-center gap-1 text-sm text-muted">
                  ...
                  {teacher.rating}
                  {teacher.reviewCount != null && ` (${teacher.reviewCount} reviews)`}
                </span>
              </div>
            </div>
          </div>
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-700 flex flex-wrap items-center gap-2">
            <Button size="sm" className="btn-primary rounded-lg" asChild>
            ...
            {teacher.website && (
              <a href={teacher.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-lg border border-slate-300 dark:border-slate-600 px-4 py-2 text-sm font-medium text-growthlab-slate hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                Website
      ...
      {teacher.recognition && teacher.recognition.length > 0 && (
      ...
      <div className="grid gap-8 lg:grid-cols-3 lg:items-start">
          ...
          {teacher.about && (
            <Card className="rounded-2xl border border-slate-200 dark:border-slate-700/50 overflow-hidden">
              <CardContent className="p-6 sm:p-8">
                ...
                <p className="text-muted text-sm leading-relaxed whitespace-pre-line">{teacher.about}</p>
              </CardContent>
            </Card>
          )}
          ...
          {teacher.howToBook && (
            <Card className="rounded-2xl border border-slate-200 dark:border-slate-700/50 overflow-hidden">
              <CardContent className="p-6 sm:p-8">
                ...
                <p className="text-muted text-sm leading-relaxed whitespace-pre-line">{teacher.howToBook}</p>
              </CardContent>
            </Card>
          )}
        ...
        <div className="space-y-6">
          {teacher.views != null && teacher.views > 0 && (
            <Card className="rounded-2xl border border-slate-200 dark:border-slate-700/50 overflow-hidden">
              <CardContent className="p-6">
                <div className="flex items-center gap-2 text-sm text-muted">
                  <Eye className="h-4 w-4 text-primary" />
                  <span><strong className="font-semibold text-growthlab-slate">{formatCount(teacher.views)}</strong> profile views</span>
                </div>
              </CardContent>
            </Card>
          )}
            ...
            <CardContent className="p-6">
              ...
              <ul className="space-y-3 text-sm">
                <li className="flex items-center gap-3">
                  ...
                  <span className="text-growthlab-slate">{teacher.location}</span>
                </li>
                {teacher.sessionsCompleted != null && (
                  <li className="flex items-center gap-3">
                    <Clock className="h-4 w-4 shrink-0 text-muted" />
                    <span className="text-growthlab-slate">{teacher.sessionsCompleted} sessions</span>
                  </li>
                )}
                {teacher.hourlyRate && (
                  <li className="flex items-center gap-3">
                    <DollarSign className="h-4 w-4 shrink-0 text-muted" />
                    <span className="text-growthlab-slate">{teacher.hourlyRate}</span>
                  </li>
                )}
                {teacher.available && <li className="font-medium text-primary">Available for sessions</li>}
                {teacher.website && (
                  <li>
                    <a href={teacher.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-primary hover:underline font-medium">
                      Visit website
              ...
              <CardContent className="p-6">
                ...
                <ul className="space-y-2">
                  {peopleAlsoViewed.map((t) => (
                    <li key={t.id}>
                      <Link href={`/teachers/${t.id}`} className="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-600 p-3 hover:border-primary/50 hover:bg-primary/[0.04] dark:hover:bg-primary/10 transition-colors">
          ...
          {similar.length > 0 && (
            <Card className="rounded-2xl border border-slate-200 dark:border-slate-700/50">
              <CardContent className="p-6">
                ...
                <h3 className="text-sm font-semibold text-growthlab-slate mb-3">Similar teachers</h3>
                <ul className="space-y-1">
                  {similar.map((t) => (
                    <li key={t.id}>
                      <Link href={`/teachers/${t.id}`} className="flex items-center justify-between gap-2 rounded-lg py-2.5 px-2 -mx-2 hover:bg-slate-50 dark:hover:bg-slate-800/50 text-growthlab-slate group transition-colors">
                        <span className="font-medium text-sm truncate group-hover:text-primary">{t.name}</span>
                        <ChevronRight className="h-4 w-4 shrink-0 text-muted" />
                      </Link>
                    </li>
                  ))}
                </ul>
                <Link href="/teachers" className="mt-3 inline-flex items-center text-sm font-medium text-primary hover:underline">
                  View all teachers
      ...
      <MobileDetailActionBar>
        <div className="flex gap-3">
          <Button size="sm" className="flex-1 min-h-11 rounded-xl btn-primary" onClick={() => setBookRequested(true)} disabled={bookRequested}>
      ...
      {teacher && (
        <TeacherProfileEditDialog
          ...
          teacher={teacher}
          onSave={(data) => {
            const next = { ...teacher, ...data } as Teacher;
            setEditedData((prev) => ({ ...prev, ...data }));
            if (id === 'me') setMyTeacherInStorage(next);
          }}
        />
      )}
    </div>
